...
TKCPCEO.Lottery.FCCQSSC.Pick.Result.prototype = {
    ...
    setRemarkValue: function(_Value, _isLengRe) {
        ...
        if (maxIndex != -1) {
            var pcP = _isLengRe != true ? this.PickClass.ListBalls[maxIndex].pYiLou : this.PickClass.ListBalls[maxIndex].pLengRe;
            pcP.innerHTML = "<b style=\"color:#EBAB00;font-weight:bold;\">" + pcP.innerHTML + "</b>";
        }
    },
    compute: function() {
        ///<summary>计算选号结果</summary>
        this.BetCount = this.PickClass.SelectBalls.length * 10;
...
TKCPCEO.Lottery.FCCQSSC.Filter.prototype = {
    ...
    getFilterBonusList: function() {
        if (pickClass.SelectBalls.length < 1) {
            alert("请先正确选择号码");
            return false;
        }
        ...
        this.PlayType = parent.TKCPCEO.Enum.prototype.Lottery.Speed.FCCQSSC.PlayType.s2ZuXuanBaoDan;
        var ball = "";
        for (var j = 0; j < pickClass.SelectBalls.length; j++) {
            ball = pickClass.SelectBalls[j].span.getAttribute("value");
            this.ListBonus.push(ball);
            this.ListClientBouns.push(ball);
            this.ListBetCount.push(10);
        }
        return true;
    },
