...
TKCPCEO.Lottery.FCCQSSC.Pick.Result.prototype = {
    ...
    compute: function() {
        ...
        this.BetCount = TKCPCEO.Math.C(3, this.PickClass.SelectBalls.length) * 6;
...
TKCPCEO.Lottery.FCCQSSC.Filter.prototype = {
    ...
    getFilterBonusList: function() {
        ...
        this.PlayType = parent.TKCPCEO.Enum.prototype.Lottery.Speed.FCCQSSC.PlayType.s3ZhiXuanZuHeFuShi;
        var ball = "";
        ...
        this.ListBetCount.push(TKCPCEO.Math.C(3, pickClass.SelectBalls.length) * 6);
...
function Event_UpdateData_YiLou() {
    ...
    for (var i = 0; i < parent.TotalLoadData.YiLou.length; i++) {
        ...
        if (dataYiLou[i].MissKey == "370") {
            outHtml(dataYiLou[i], "三星(不定位)遗漏", "三星组选（百、十、个位不排序）遗漏。", "Trend.aspx?tid=j18", 0, htmlStandard);
        }
