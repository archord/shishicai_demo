...
TKCPCEO.Lottery.FCCQSSC.Pick.Result.prototype = {
    ...
    setRemarkValue: function(_Value) {
        ...
        if (_arrayListValue.length != 10) { return; }
...
TKCPCEO.Lottery.FCCQSSC.Filter.prototype = {
    ...
    getFilterBonusList: function() {
        ...
        if (this.Enable == true) {
            this.PlayType = parent.TKCPCEO.Enum.prototype.Lottery.Speed.FCCQSSC.PlayType.s5DanShi;
            ///SilverLight过滤
        } else {
            this.PlayType = parent.TKCPCEO.Enum.prototype.Lottery.Speed.FCCQSSC.PlayType.s5DanShi;
            var ball10000 = "";
            for (var i = 0; i < pickClass10000.SelectBalls.length; i++) {
                ball10000 += pickClass10000.SelectBalls[i].span.getAttribute("value");
            }
            if (pickClass10000.SelectBalls.length > 1) {
                this.PlayType = parent.TKCPCEO.Enum.prototype.Lottery.Speed.FCCQSSC.PlayType.s5ZuHe;
            }

            var ball1000 = "";
            for (var i = 0; i < pickClass1000.SelectBalls.length; i++) {
                ball1000 += pickClass1000.SelectBalls[i].span.getAttribute("value");
            }
...
function PickBallsByYiLouData(_typeYiLou, _key) {
    ...
    function checkBallCondition(_value, _ballValue, _indexBall) {
        ...
        switch (_typeYiLou) {
            case "601": ///前两位遗漏
                isValidate = _indexBall > 1 || (xintTai[_indexBall] != null && xintTai[_indexBall] == _value);
                break;
            case "201": ///后两位遗漏
                _indexBall = _indexBall - 3;
                isValidate = _indexBall < 0 || (xintTai[_indexBall] != null && xintTai[_indexBall] == _value);
                break;
            case "701": ///前三位遗漏
                isValidate = _indexBall > 2 || (xintTai[_indexBall] != null && xintTai[_indexBall] == _value);
                break;
            case "801": ///中三位遗漏
                _indexBall = _indexBall - 1;
                isValidate = _indexBall < 0 || _indexBall > 2 || (xintTai[_indexBall] != null && xintTai[_indexBall] == _value);
                break;
            case "301": ///后三位遗漏
...
function Event_UpdateData_YiLou() {
    ...
    if (parent.TotalLoadData == null) { setTimeout(function() { Event_UpdateData_YiLou(); }, 1000); return; }; var htmlStandard = new Array(); ///标准遗漏
