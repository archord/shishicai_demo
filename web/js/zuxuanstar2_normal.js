...
TKCPCEO.Lottery.FCCQSSC.Pick.Result.prototype = {
    ...
    compute: function() {
        ...
        this.BetCount = TKCPCEO.Math.C(2, this.PickClass.SelectBalls.length);
...
TKCPCEO.Lottery.FCCQSSC.Pick.prototype = {
    ...
    clickBall: function(_ballItem, _isCompute) {
        ...
        if (this.SelectBalls.exists(_ballItem)) {
            ...
            if (this.SelectBalls.length >= 7 && _isCompute == true) {
...
TKCPCEO.Lottery.FCCQSSC.Filter.prototype = {
    ...
    getFilterBonusList: function() {
        ...
        if (pickClass.SelectBalls.length < 2) {
            alert("请先正确选择号码");
            return false;
        }
        ...
        if (this.Enable == true) {
            this.PlayType = parent.TKCPCEO.Enum.prototype.Lottery.Speed.FCCQSSC.PlayType.s2ZuXuanDanShi;
            ...
            if (pickClass.SelectBalls.length > 2) {
                this.PlayType = parent.TKCPCEO.Enum.prototype.Lottery.Speed.FCCQSSC.PlayType.s2ZuXuanFuShi;
                this.ListBonus.push(ball.split("").join(","));
            } else {
                this.ListBonus.push("_,_,_," + ball.split("").join(","));
            }
            this.ListClientBouns.push(ball.split("").join(","));
            this.ListBetCount.push(TKCPCEO.Math.C(2, pickClass.SelectBalls.length));
        }
        return true;
    },
...
function Event_UpdateData_YiLou() {
    ...
    function fillFenWeiYiLou(_dataYiLouItem) {
        for (var i = 0; i < _dataYiLouItem.MissValue.length; i++) {
            fenWeiYiLou[parseInt(_dataYiLouItem.MissValue[i].MissKey, 10)] = parseInt(_dataYiLouItem.MissValue[i].MissValue, 10);
        }
    }
    ...
    for (var i = 0; i < parent.TotalLoadData.YiLou.length; i++) {
        ...
        if (dataYiLou[i].MissKey == "270") {
            outHtml(dataYiLou[i], "二星(不定位)遗漏", "二星组选（十、个位不排序）遗漏", "Trend.aspx?tid=i14", 0, htmlStandard);
        }
        ...
        ///分位遗漏
        if (dataYiLou[i].MissKey == "272") {///胆码形态排序
