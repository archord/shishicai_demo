...
TKCPCEO.Lottery.FCCQSSC.Pick.Result.prototype = {
    ...
    compute: function() {
        ...
        this.BetCount = TKCPCEO.Math.C(1, this.PickClass1.SelectBalls.length) * TKCPCEO.Math.C(1, this.PickClass10.SelectBalls.length) * TKCPCEO.Math.C(1, this.PickClass100.SelectBalls.length);
    ...
    initialize: function() {
        ///<summary>初始化</summary>
        if (this.PickInfo.BetCount == null || this.PickInfo.BetMoney == null || this.PickClass1 == null || this.PickClass10 == null || this.PickClass100 == null) {
            alert("初始化前请先做好准备工作");
            return;
        }
    },
...
TKCPCEO.Lottery.FCCQSSC.Filter.prototype = {
    ...
    getFilterBonusList: function() {
        ...
        if (pickClass1.SelectBalls.length < 1 || pickClass10.SelectBalls.length < 1 || pickClass100.SelectBalls.length < 1) {
            alert("请先正确选择号码");
            return false;
        }
        ...
        if (this.Enable == true) {
            ...
            for (var i = 0; i < pickClass100.SelectBalls.length; i++) {
                ball100 += pickClass100.SelectBalls[i].span.getAttribute("value");
            }
            if (pickClass100.SelectBalls.length > 1) {
                this.PlayType = parent.TKCPCEO.Enum.prototype.Lottery.Speed.FCCQSSC.PlayType.s3ZuHe;
            }
            var ball10 = "";
            for (var i = 0; i < pickClass10.SelectBalls.length; i++) {
            ...
            this.ListBetCount.push(pickClass100.SelectBalls.length * pickClass10.SelectBalls.length * pickClass1.SelectBalls.length);
        }
        return true;
    },
...
TKCPCEO.Lottery.FCCQSSC.UpdateData.prototype = {
    ...
    this.Container.UpdateDataNormalYiLou = $("ContainerNormalYiLou");
        ...
        menus[3].wrap = this.Container.UpdateDataWanNengFuShi;
        menus[4].wrap = this.Container.UpdateData10Trend.parentNode;
...
function getBonusResultList(_bonusNumber) {
    ...
    var dx = (ball3 >= 5 ? "大" : "小") + (ball2 >= 5 ? "大" : "小") + (ball1 >= 5 ? "大" : "小");
    var jo = (ball3 % 2 == 1 ? "奇" : "偶") + (ball2 % 2 == 1 ? "奇" : "偶") + (ball1 % 2 == 1 ? "奇" : "偶");
    var zh = (PrimeArray.exists(ball3) ? "质" : "合") + (PrimeArray.exists(ball2) ? "质" : "合") + (PrimeArray.exists(ball1) ? "质" : "合");
    var c3ys = ball3 % 3 + "" + ball2 % 3 + "" + ball1 % 3;
    var xt = ball3 == ball2 && ball3 == ball1 ? "豹子" : (ball3 != ball2 && ball3 != ball1 && ball2 != ball1 ? "组6" : "组3");
    listR.push(dx);
    listR.push(jo);
    listR.push(zh);
    listR.push(c3ys);
...
function PickBallsByYiLouData(_typeYiLou, _key) {
    ...
    function checkBallCondition(_value, _ballValue, _indexBall) {
        ...
        switch (_typeYiLou) {
            case "314": ///单胆遗漏
                isValidate = xintTai == "_" || parseInt(xintTai, 10) == intValue;
                break;
            case "315": ///双胆遗漏
                isValidate = xintTai == "_" || parseInt(xintTai, 10) == intValue;
                break;
            ...
            case "307": ///奇偶遗漏
                switch (xintTai) {
                    case "奇":
                        isValidate = intValue % 2 == 1;
                        break;
                    case "偶":
                        isValidate = intValue % 2 == 0;
                        break;
                }
                break;
            case "309": ///质合遗漏
                switch (xintTai) {
                    case "质":
                        isValidate = PrimeArray.exists(intValue);
                        break;
                    case "合":
                        isValidate = !PrimeArray.exists(intValue);
                        break;
            ...
            case "316": ///012路遗漏
                ...
                break;
            case "318": ///四码复式
                isValidate = _ballValue.split("").exists(_value);
                break;
            case "319": ///五码复式
                isValidate = _ballValue.split("").exists(_value);
                break;
...
function Event_UpdateData_YiLou() {
    ...
    for (var i = 0; i < parent.TotalLoadData.YiLou.length; i++) {
        ...
        if (dataYiLou[i].MissKey == "301") {
            outHtml(dataYiLou[i], "三星(定位)遗漏", "三星（百位、十位、个位）遗漏", "Trend.aspx?tid=e15", 0, htmlStandard);
        }
        if (dataYiLou[i].MissKey == "302") {
            outHtml(dataYiLou[i], "三星豹子遗漏", "三星的豹子号（百、十、个位相同）遗漏", "Trend.aspx?tid=e20", 1, htmlStandard);
        }
        if (dataYiLou[i].MissKey == "303") {
            outHtml(dataYiLou[i], "组三类型遗漏", "三星的组三号码（包含对子的号码）遗漏", "", 2, htmlStandard);
        }
        if (dataYiLou[i].MissKey == "314") {
            outHtml(dataYiLou[i], "三星单胆遗漏", "三星的单胆（定一胆，其它位全包）遗漏", "", 3, htmlStandard);
        }
        if (dataYiLou[i].MissKey == "315") {
            outHtml(dataYiLou[i], "三星双胆遗漏", "三星的双胆（定两胆，其它位全包）遗漏", "", 4, htmlStandard);
        }

        ///形态遗漏
        if (dataYiLou[i].MissKey == "305") {
            outHtml(dataYiLou[i], "三星大小遗漏", "三星（百、十、个位）的大小形态遗漏", "", 0, htmlXingTai);
        }
        if (dataYiLou[i].MissKey == "307") {
            outHtml(dataYiLou[i], "三星奇偶遗漏", "三星（百、十、个位）的奇偶形态遗漏", "", 1, htmlXingTai);
        }
        if (dataYiLou[i].MissKey == "309") {
            outHtml(dataYiLou[i], "三星质合遗漏", "三星（百、十、个位）的质合形态遗漏", "", 2, htmlXingTai);
        }
        if (dataYiLou[i].MissKey == "316") {
            outHtml(dataYiLou[i], "三星012路遗漏", "三星（百、十、个位）的012路形态遗漏", "", 3, htmlXingTai);
        }

        ///万能复式遗漏
        if (dataYiLou[i].MissKey == "318") {
            outHtml(dataYiLou[i], "四码遗漏", "三星（百、十、个位）的万能四码（4个号码为一组，共30组）遗漏", "", 0, htmlWanNengFuShi, 0);
        }
        if (dataYiLou[i].MissKey == "319") {
            outHtml(dataYiLou[i], "五码遗漏", "三星（百、十、个位）的万能五码（5个号码为一组，共17组）遗漏", "", 1, htmlWanNengFuShi, 0);
        }
        if (dataYiLou[i].MissKey == "320") {
            outHtml(dataYiLou[i], "六码遗漏", "三星（百、十、个位）的万能六码（6个号码为一组，共10组）遗漏", "", 2, htmlWanNengFuShi, 0);
        }
        if (dataYiLou[i].MissKey == "321") {
            outHtml(dataYiLou[i], "七码遗漏", "三星（百、十、个位）的万能七码（7个号码为一组，共6组）遗漏", "", 3, htmlWanNengFuShi, 0);
        }
...
function Event_UpdateData_Bonus(_actionOrder) {
    ...
    html += ">期号" + (orderDesc ? parent.TextOrderCharactor.Desc : parent.TextOrderCharactor.Asc) + "</th><th>开奖号码</th><th>大小</th><th>奇偶</th><th>质合</th><th>012路</th><th>形态</th></tr>";
    var htmlSort = "";
    for (var i = 0; i < bonusStrAry.length - 2; i++) {
        ...
        var addString = "<tr><td>" + issue.split("-")[1] + "期</td><td class=\"n\">" + bonus5 + bonus4 + "<strong>" + bonus3 + bonus2 + bonus1 + "</strong>" + "</td><td>" + listR[0] + "</td><td>" + listR[1] + "</td><td>" + listR[2] + "</td><td>" + listR[3] + "</td><td>" + listR[4] + "</td></tr>";
        if (orderDesc) {
            html += addString;
        } else {
