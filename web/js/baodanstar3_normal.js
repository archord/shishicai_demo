...
TKCPCEO.Lottery.FCCQSSC.Pick.Result.prototype = {
    ...
    setRemarkValue: function(_Value, _isLengRe) {
        ...
        for (var i = 0; i < _arrayListValue.length; i++) {
            ...
            //最大值
            if (_isLengRe == true) {
                this.PickClassFS.ListBalls[i].pLengRe.innerHTML = IsEmpty ? "&nbsp;" : fenweiAry;

                this.PickClassDS1.ListBalls[i].pLengRe.innerHTML = IsEmpty ? "&nbsp;" : fenweiAry;
            } else {
                this.PickClassFS.ListBalls[i].pYiLou.innerHTML = IsEmpty ? "&nbsp;" : fenweiAry;
                this.PickClassDS1.ListBalls[i].pYiLou.innerHTML = IsEmpty ? "&nbsp;" : fenweiAry;
            }
        }
        if (_isLengRe != true) {
            if (maxIndex != -1) {
                this.PickClassFS.ListBalls[maxIndex].pYiLou.innerHTML = "<b style=\"color:#EBAB00;font-weight:bold;\">" + this.PickClassFS.ListBalls[maxIndex].pYiLou.innerHTML + "</b>";
                this.PickClassDS1.ListBalls[maxIndex].pYiLou.innerHTML = "<b style=\"color:#EBAB00;font-weight:bold;\">" + this.PickClassDS1.ListBalls[maxIndex].pYiLou.innerHTML + "</b>";
            }
        } else {
            if (maxIndex[0] != -1) {
                this.PickClassFS.ListBalls[maxIndex].pLengRe.innerHTML = "<b style=\"color:#EBAB00;font-weight:bold;\">" + this.PickClassFS.ListBalls[maxIndex].pLengRe.innerHTML + "</b>";
                this.PickClassDS1.ListBalls[maxIndex].pLengRe.innerHTML = "<b style=\"color:#EBAB00;font-weight:bold;\">" + this.PickClassDS1.ListBalls[maxIndex].pLengRe.innerHTML + "</b>";
            }
        }
    },
    compute: function() {
        ...
        if (this.IsFuShi) {
            this.BetCount = (this.PickClassDS10.SelectBalls.length != 0 && this.PickClassDS1.SelectBalls.length != 0) ? (this.PickClassDS10.SelectBalls[0].span.getAttribute("value") == this.PickClassDS1.SelectBalls[0].span.getAttribute("value") ? 10 : 10) : 0;
        } else {
            this.BetCount = this.PickClassFS.SelectBalls.length == 1 ? 55 : 0;
        }
...
TKCPCEO.Lottery.FCCQSSC.Filter.prototype = {
    ...
    getFilterBonusList: function() {
        ...
        if (PickR.IsFuShi) {
            if (pickClassDS10.SelectBalls.length != 1 || pickClassDS1.SelectBalls.length != 1) {
                alert("请选两个胆码");
                return false;
            }
        } else {
            if (pickClassFS.SelectBalls.length != 1) {
                alert("请选择一个胆码");
                return false;
            }
        }
        ...
        this.PlayType = parent.TKCPCEO.Enum.prototype.Lottery.Speed.FCCQSSC.PlayType.s3ZuXuanBaoDan;
        var ball = "";
        if (PickR.IsFuShi) {
            ball = PickR.PickClassDS10.SelectBalls[0].span.getAttribute("value") + "," + PickR.PickClassDS1.SelectBalls[0].span.getAttribute("value");
            this.ListBetCount.push(PickR.PickClassDS10.SelectBalls[0].span.getAttribute("value") == PickR.PickClassDS1.SelectBalls[0].span.getAttribute("value") ? 10 : 10);
        } else {
            ball = PickR.PickClassFS.SelectBalls[0].span.getAttribute("value");
            this.ListBetCount.push(55);
        }
        this.ListBonus.push(ball);
        this.ListClientBouns.push(ball);
        return true;
    },
    ...
    initialize: function() {
        ///<summary>初始化</summary>
        if (this.Container.PickInfo == null) {
            alert("请先初始化相关信息");
            return;
        }
...
//        }
    ...
    PickR.setRemarkValue(fenWeiYiLou.toString());
