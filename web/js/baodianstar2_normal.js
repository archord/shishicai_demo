...
TKCPCEO.Lottery.FCCQSSC.Pick.prototype = {
    ...
    submitTicket: function() {
        ...
        this.PlayType = parent.TKCPCEO.Enum.prototype.Lottery.Speed.FCCQSSC.PlayType.s2ZuXuanHeZhi;
...
function Event_UpdateData_YiLou() {
    ...
    for (var i = 0; i < parent.TotalLoadData.YiLou.length; i++) {
        if (dataYiLou[i].MissKey == "240") {
            outHtml(dataYiLou[i], "二星和值遗漏", "十位与个位的和值（此处统计范围为4-14共11个和值）遗漏", "Trend.aspx?tid=g5", 0, htmlStandard, 0);
        }

        if (dataYiLou[i].MissKey == "242") {
            outHtml(dataYiLou[i], "二星和值尾遗漏", "十位与个位的和值尾数（0-9）遗漏", "", 1, htmlStandard, 0);
        }

        if (dataYiLou[i].MissKey == "243") {
            outHtml(dataYiLou[i], "二星和值段遗漏", "十位与个位的和值段（第一段：0-6第二段：6-9第三段：9-11第四段：12-18）遗漏", "", 2, htmlStandard, 0);
        }

        if (dataYiLou[i].MissKey == "241") {///和值分位遗漏
            var maxPercent = 0;
...
function Event_UpdateData_Bonus() {
    ...
    for (var i = 0; i < bonusStrAry.length - 2; i++) {
        ...
        sumAry[i] = parseInt(bonusStrAry[i].split("|")[1].toString().substr(3, 1), 10) + parseInt(bonusStrAry[i].split("|")[1].toString().substr(4, 1), 10);
    ...
    html += "<tr><td>开奖号码</td><td>" + bonus2Ary[0] + "," + bonus1Ary[0] + "</td><td>" + bonus2Ary[1] + "," + bonus1Ary[1] + "</td><td>" + bonus2Ary[2] + "," + bonus1Ary[2] + "</td><td>" + bonus2Ary[3] + "," + bonus1Ary[3] + "</td><td>" + bonus2Ary[4] + "," + bonus1Ary[4] + "</td><td>" + bonus2Ary[5] + "," + bonus1Ary[5] + "</td><td>" + bonus2Ary[6] + "," + bonus1Ary[6] + "</td><td>" + bonus2Ary[7] + "," + bonus1Ary[7] + "</td><td>" + bonus2Ary[8] + "," + bonus1Ary[8] + "</td><td>" + bonus2Ary[9] + "," + bonus1Ary[9] + "</td></tr>";
